...
class VndaMonitor(MonitorBase):
    logger = logging.getLogger(__name__)

    def __init__(self,
                 symbol = "VNDA",
                 press_release_url = "https://www.vandapharma.com/investors?locale=en/#news"
                 ):
    ...
    def fetch_news_articles(self, driver=None):
        ...
        articles = WebDriverWait(driver, 10).until(
            EC.presence_of_all_elements_located((
                By.XPATH,
                "//section[@id='recentNews']/div/div/div[2]/div"
            )))
        self.logger.debug(f"Found {len(articles)} articles on page")
        article_data = []
        for article in articles:
            try:
                date = article.find_element(By.TAG_NAME, "h6").text.strip()
